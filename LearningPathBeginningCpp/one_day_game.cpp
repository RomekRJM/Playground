...
Weapon* GameWorld::generateWeapon() {
    ...
    currentItem = new Weapon(weaponNames[round-1], minDamage, maxDamage);
...
Character* GameWorld::generateOpponent() {
    ...
    currentOponent = new Character(hp, damage, oponentNames[round-1]);
...
void redrawScreen(GameWorld* world, Character* player) {
    // clear screen and set cursor at the begining of new screen
    ...
    switch (world->getGameState()) {
        ...
        case stats_screen:
            ...
            break;
        case round_won:
            description = "You won!!!\n\nYou have leveled up and your "
                    "base damage raises by 5.\n"
                    "You can pick his weapon and replace yours OR\n"
                    "sell his weapon and use the money to train at the gym (+50% base hp).\n"
                    "You will be fully healed, no matter what you choose."
                    "\n\nHis weapon \n\n" + world->getOponent()->getWeapon()->toString()
                    + "\n\nYour weapon \n\n" + player->getWeapon()->toString();

            options = "\n\nPress 'y' replace your current weapon.\n"
                    "Press 'h' to exchange oponent's weapon for 50% base hp.\n";
            break;
        case game_over:
...
void simulateFight(Character* player, Character* oponent) {
    while (player->getHp() > 0 && oponent->getHp() > 0) {
        ...
        cout << "Player hp: " << player->getHp() << " oponent hp: " 
                << oponent->getHp() << endl;
    }
}
